@section('auth-form')
    ...
    <button class="flex items-center justify-center w-full h-10 text-sm rounded-full bg-brand text-white hover:bg-brand-600 transition mb-8 focus" title="{{ __('Enter the sample') }}" aria-label="{{ __('Enter the sample') }}" role="button">{{ __('Enter the sample') }}</button>
@endsection
