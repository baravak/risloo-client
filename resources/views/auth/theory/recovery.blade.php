@section('auth-form')
    ...
    <button class="flex items-center justify-center w-full h-10 text-sm rounded-full bg-brand text-white hover:bg-brand-600 transition mb-8 focus" title="{{ __('Change password' )}}">{{ __('Change password' )}}</button>
@endsection

@section('auth-nav')
    <div class="flex justify-center">
        <a href="{{ route('auth') }}" class="text-sm" title="{{ __('Login') }}">{{ __('Login') }}</a>
        <span class="px-4 text-gray-500">|</span>
        <a href="{{ route('register') }}" class="text-sm" title="{{ __('Register') }}">{{ __('Register') }}</a>
    </div>
@endsection
