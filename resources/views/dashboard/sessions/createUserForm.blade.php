<div class="m-auto w-full md:w-1/2 mt-4">
    <form class="w-full" method="POST" action="{{ route('dashboard.schedules.booking', $session->id) }}">
        @method('POST')
        <div class="border border-gray-300 rounded p-4">
            @can('admin', $room)
                <div>
                    <label for="client_id" class="block mb-2 text-sm text-gray-700 font-medium">{{ __('Client') }}</label>
                    <select name="client_id" id="client_id" data-url="{{route('dashboard.room.users.index', ['room' => $room->id, 'not_in_session' => $session->id])}}" class="select2-select">
                    </select>
                </div>
            @endcan
            <div class="mt-4">
