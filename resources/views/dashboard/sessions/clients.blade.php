<div>
    <div class="mb-4">
        <h3 class="flex items-center font-bold text-gray-700 cursor-default">
            ...
            <span class="text-xs text-gray-600 font-light mr-2" data-xhr="total">({{$session->clients ? $session->clients->whereIn('position', ['clients'])->count() : 0}})</span>
        </h3>
    </div>
    <div class="grid grid-cols-1 2xl:grid-cols-2 gap-2">
        @foreach ($session->clients ? $session->clients->whereIn('position', ['clients']): [] as $client)
