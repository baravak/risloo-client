<div>
    <div class="relative mb-4">
        <div>
            <h3 class="flex items-center font-bold text-gray-700 cursor-default">
                ...
                <span class="text-xs text-gray-600 font-light mr-2">({{ $case->clients ? $case->clients->count() : 0 }})</span>
            </h3>
        </div>
        @can('manager', $case)
            <div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-2">
        @foreach ($case->clients as $client)
            <div class="client-record flex items-center border border-gray-200 rounded px-3 py-2 hover:bg-gray-50 transition cursor-default" data-xhr="client-{{ $client->user->id }}" data-id="{{ $client->user->id }}">
