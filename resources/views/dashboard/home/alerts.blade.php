@if ($user->no_password)
    <div class="flex-col xl:flex-row flex xl:items-center xl:justify-between border border-gray-300 rounded p-4">
        ...
        <div class="mt-4 xl:mt-0">
            <a href="{{ route('dashboard.users.me.edit') }}#password-tab" class="px-4 py-1 text-sm text-brand hover:text-white border border-brand hover:bg-brand rounded-full transition">تنظیم کلمه عبور</a>
        </div>
    </div>
@endif
@if (!$user->centers)
    <div class="flex-col xl:flex-row flex xl:items-center xl:justify-between border border-gray-300 rounded p-4 mt-4">
        ...
        <div class="mt-4 xl:mt-0">
            <a href="{{ route('dashboard.centers.index') }}" class="px-4 py-1 text-sm text-brand hover:text-white border border-brand hover:bg-brand rounded-full transition">مراکز درمانی</a>
        </div>
    </div>
@endif
