...
window.encryptPublicLong = encryptPublicLong;
/**
 * rsa私钥解密长字符串
 * @param str
 * @param privateKey
...
function decryptPrivateLong(text, privateKey) {
    ...
    try {
        if (text.length > maxLength) {
            var ct1_1 = "";
            var lt = text.match(/.{1,512}/g);
            if (lt) {
                lt.forEach(function (entry) {
                    ct1_1 += key.decrypt(entry);
                });
            }
            return ct1_1;
        }
...
async function asyncDecryptPrivateLong(){
    return decryptPrivateLong(...arguments);
}

window.decryptPrivateLong = decryptPrivateLong;
